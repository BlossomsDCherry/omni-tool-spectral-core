...
MEMORY
    ...
    OFS_OSIS_MEMORY ( rw ) : ORIGIN = (16842776), LENGTH = (28)
 ...
 *(".kernel.*")
 ...
 net_buf_pool_area : ALIGN_WITH_INPUT { _net_buf_pool_list_start = .; KEEP(*(SORT_BY_NAME(._net_buf_pool.static.*))); _net_buf_pool_list_end = .;; } > RAM AT > FLASH
    __data_region_end = .;
.option_setting_osis (16842776) : ALIGN_WITH_INPUT
{
...
.intList :
{
 ...
 KEEP(*(.gnu.attributes))
    ...
    OFS_OSIS_MEMORY (NOLOAD) : { __OFS_OSIS_MEMORY_start = .; KEEP(*(OFS_OSIS_MEMORY)) KEEP(*(OFS_OSIS_MEMORY.*)) __OFS_OSIS_MEMORY_end = .; } > OFS_OSIS_MEMORY __OFS_OSIS_MEMORY_size = __OFS_OSIS_MEMORY_end - __OFS_OSIS_MEMORY_start; __OFS_OSIS_MEMORY_load_start = LOADADDR(OFS_OSIS_MEMORY);
.last_section :
{
  KEEP(*(.last_section))
} > FLASH
