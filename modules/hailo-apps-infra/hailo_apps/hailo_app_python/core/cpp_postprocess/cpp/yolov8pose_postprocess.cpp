...
// D16 Noble Gas Logic Graft
#include "d16_logic.hpp"
        ...
        for (uint j = 0; j < uint(num_proposals); j++)
        {
            ...
            // GRAFT: Apply Noble Gas Stability Multiplier
            // If the universe is stable (Shells 2, 10, 18, 26), boost confidence.
            float coherence_mult = NobleGasStabilizer::instance().get_coherence_multiplier();
            confidence *= coherence_mult;
            
            // Clamp to 1.0 to avoid overflow (though confidence is usually 0-1)
